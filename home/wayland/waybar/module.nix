...
let
  ...
  cfg = config.programs.waybar-tray;

  # Used when generating warnings
  modulesPath = "programs.waybar-tray.settings.[].modules";
...
in
{
  ...
  options.programs.waybar-tray = with lib.types; {
  ...
  config =
      ...
      configSource =
        let
          ...
          makeConfiguration = configuration:
            ...
            in
            removeNulls (settingsWithoutModules // settingsModules);
          # The clean list of configurations
          finalConfiguration = map makeConfiguration cfg.settings;
        in
        writePrettyJSON "waybar-config.json" finalConfiguration;
        ...
        let
          ...
          allWarnings = flip concatMap allFaultyModules
            ({ settings, undef, unref, invalidName }:
              ...
              in
              undefined ++ unreferenced ++ invalid);
        in
        allWarnings;
    ...
    mkIf cfg.enable (mkMerge [
      {
        assertions = [
          (lib.hm.assertions.assertPlatform "programs.waybar-tray" pkgs
            lib.platforms.linux)
        ];
